...
# The function checks if the URL is a wiki address
def wiki_check(url: str) -> Optional[Match[str]]:
    return re.search("(https://).*(wiki).*(/wiki/)", url)


# The function returns the lang prefix from the URL
def lang_prefix(url: str) -> str:
    return re.search(r"(https://)(.*?)(.wiki)", url).group(2)


# The function returns urls from url by content
def get_urls(page_content):
    beautiful_oup = BeautifulSoup(page_content, 'html.parser')
    page_content = beautiful_oup.find(id="bodyContent")
    if page_content:
        return np.array([urllib.parse.unquote_plus(url['href']) for url in page_content.find_all('a', href=True)])
    return np.empty()


# The function returns full url in wiki format
def wiki_url(url: str, lang_prefix: str):
    if 'https' not in url:
...
def find_wiki_path(start: str, finish: str, rate_limit: int, depth: int, lang_prefix: str) -> object:
    ...
    while not tmp_urls.empty():
        ...
        while not f_urls.empty():
            ...
            if content:
                for n_url in get_urls(content):
                    n_url = wiki_url(n_url, lang_prefix)
...
def main() -> None:
    ...
    if not validators.url(args.start) or not validators.url(args.finish) or not wiki_check(args.start) \
            or not wiki_check(args.finish):
        print("Valid/s not valid")
        exit(1)

    start_prefix = lang_prefix(args.start)
    finish_prefix = lang_prefix(args.finish)
